...
function endGame() {
  ...
  // ส่งคะแนนจริงเข้า backend เสมอ ไม่ว่าจะผ่านเป้าหมายท้องถิ่น (100) หรือไม่
  (async () => {
    try {
...
function restartGame() {
  ...
  zooSessionPromise = ZooPassport.startMiniGame()
    ...
    .catch((err) => console.error('[ZooPassport] เริ่มเกมใหม่ไม่สำเร็จ:', err));

  startTimers();
}

// เดิมปุ่มนี้แค่ toggle แสดง/ซ่อนคอลเลกชันแสตมป์ปลอมในเกมเอง (ไม่เคยพากลับแอปจริง)
// ตอนนี้พากลับแอปจริงเหมือนปุ่ม "ดูพาสปอต" ของเกมอื่นทุกตัว
...
function startGame(message) {
  ...
  // เกมเริ่มจริงตรงนี้ (อัตโนมัติเมื่อกล้อง/ระบบตรวจจับพร้อม) — ขอ session token ตอนนี้
  zooSessionToken = null;
  zooSubmitResult = null;
  zooSessionPromise = ZooPassport.startMiniGame()
    .then((r) => { zooSessionToken = r.sessionToken; })
