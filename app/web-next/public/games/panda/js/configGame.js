// Zoo Passport integration — เก็บ session token ไว้ตั้งแต่เริ่มเกม
// เอาไปใช้ตอน submit คะแนนหลังเคลียร์ครบ 5 ด่าน (ดู startGame/showFinalCompletion)
...
class Player {
    ...
    die() {
        ...
        if (game.lives <= 0) {
            ...
            // แพ้กลางเกม (ชีวิตหมดก่อนครบ 5 ด่าน) — ก็ต้องส่งคะแนนเข้า backend
            // เหมือนกัน ไม่งั้นรอบที่แพ้จะไม่ถูกบันทึกเลย (เดิมส่งแค่ตอนชนะครบ)
            (async () => {
                try {
                    if (zooSessionPromise) await zooSessionPromise;
                    if (!zooSessionToken) throw new Error("ไม่มี session token — เริ่มเกมใหม่อีกครั้ง");
                    zooSubmitResult = await ZooPassport.submitMiniGame(zooSessionToken, game.score);
                } catch (err) {
...
function startGame() {
    ...
    // ขอ session token ใหม่ทุกครั้งที่เริ่มเกม (เล่นใหม่ = attempt ใหม่)
...
function showFinalCompletion() {
    ...
    // ส่งคะแนนจริงเข้า backend — รอ session token ให้พร้อมก่อน (เผื่อ start ยังไม่เสร็จ)
    (async () => {
        try {
            if (zooSessionPromise) await zooSessionPromise;
            if (!zooSessionToken) throw new Error("ไม่มี session token — เริ่มเกมใหม่อีกครั้ง");
...
function showPassport() {
    // พากลับเข้าแอปหลัก — ไปหน้ารับตราถ้าเพิ่งปลดล็อกครั้งแรก ไม่งั้นไปหน้าโซนเฉยๆ
