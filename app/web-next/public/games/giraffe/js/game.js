...
// ============================================================================
// ปุ่ม "ดูพาสปอต" — เชื่อมผ่าน shared client (public/games/_shared/zoo-passport-client.js)
// เกมนี้ served มาจาก public/games/giraffe/ ของแอป Next.js เอง (same-origin
// กับตอน login) เลยกลับเข้าแอปตรงๆ ได้โดยไม่ต้องใช้ postMessage/LIFF แยก
// ============================================================================
function goToPassport() {
    ZooPassport.goBackToApp(zooSubmitResult);
}

// เดิมฟังก์ชันนี้ล็อกปุ่ม "ดูพาสปอต" ไว้จนกว่า bestScore (ท้องถิ่น) จะถึง 5000
// ตอนนี้เปิดปุ่มไว้เสมอหลังเล่นจบ — backend (mini_games.pass_score) เป็นคน
// ตัดสิน pass/fail จริง ไม่ใช่เกณฑ์ท้องถิ่นนี้ (ดู finishGame -> submitMiniGame)
function updatePassportButton() {
    ...
    passportBtn.setAttribute('aria-disabled', 'false');
    passportBtn.classList.remove('locked');

    if (passportHint) {
        passportHint.classList.add('hidden');
    }
}
...
function finishGame() {
    ...
    // ส่งคะแนนจริงเข้า backend เสมอ ไม่ว่าจะผ่านเกณฑ์ท้องถิ่น (5000) หรือไม่
    // backend เป็นคนตัดสิน pass/fail จริงผ่าน mini_games.pass_score
    (async () => {
        try {
            if (zooSessionPromise) await zooSessionPromise;
            if (!zooSessionToken) throw new Error('ไม่มี session token — เริ่มเกมใหม่อีกครั้ง');
            zooSubmitResult = await ZooPassport.submitMiniGame(zooSessionToken, totalScore);
        } catch (err) {
